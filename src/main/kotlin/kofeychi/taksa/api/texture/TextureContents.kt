...
class TextureContents(
    ...
    fun fill(value: Int = 0): TextureContents {
        require(dataType == TextureDataType.UBYTE) {
            "fill(Int) is only available for UBYTE texture contents."
        }
        repeat(size) { data.put(it, value.toByte()) }
        return this
    }

    fun setPixel(x: Int, y: Int, vararg components: Int): TextureContents {
        return set(x, y, *components)
    }

    fun set(x: Int, y: Int, vararg components: Int): TextureContents {
        require(dataType == TextureDataType.UBYTE) {
            "set(Int...) is only available for UBYTE texture contents."
        }
        require(components.size == channels) {
            "Expected $channels component(s), got ${components.size}."
        }

        val byteOffset = index(x, y)
        components.forEachIndexed { i, component ->
            data.put(byteOffset + i, component.coerceIn(0, 255).toByte())
        }
        return this
    }
    ...
    fun pixel(x: Int, y: Int): IntArray {
        require(dataType == TextureDataType.UBYTE) {
            "pixel() is only available for UBYTE texture contents."
        }
        val byteOffset = index(x, y)
    ...
    fun transform(
        action: (x: Int, y: Int, pixel: IntArray) -> IntArray
    ): TextureContents {
        require(dataType == TextureDataType.UBYTE) {
            "transform() is only available for UBYTE texture contents."
        }

        for (y in 0 until height) {
            for (x in 0 until width) {
                val replacement = action(x, y, pixel(x, y))
                set(x, y, *replacement)
            }
        }
        return this
    ...
    fun setFloat(x: Int, y: Int, vararg components: Float): TextureContents {
        require(dataType == TextureDataType.FLOAT) {
            "setFloat(Float...) requires FLOAT texture contents."
        }
        require(components.size == channels) {
            "Expected $channels component(s), got ${components.size}."
        }
    ...
    companion object {
        fun create(
            ...
            return TextureContents(width, height, format, dataType).apply(action)
        }

        fun rgba(width: Int, height: Int): TextureContents {
            return TextureContents(width, height, TextureFormat.RGBA, TextureDataType.UBYTE)
        }
    }
}
